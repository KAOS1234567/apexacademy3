...
type PlayerRow = { first_name: string; last_name: string; position: string | null; jersey_number: number | null };
...
export function ReportDocument() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setAllPlayers(allP.map((pl) => ({ first_name: pl.first_name, last_name: pl.last_name, position: pl.position, jersey_number: pl.jersey_number })));
    ...
    <div className="print-page">
      ...
      <section className="report-section">
        ...
        <div className="report-stats">
          <div className="report-stat"><span className="report-stat-label">اللاعبون</span><span className="report-stat-value">{String(counts.players).padStart(2, "0")}</span></div>
          <div className="report-stat"><span className="report-stat-label">الفرق</span><span className="report-stat-value">{String(counts.teams).padStart(2, "0")}</span></div>
          <div className="report-stat"><span className="report-stat-label">الطاقم</span><span className="report-stat-value">{String(counts.staff).padStart(2, "0")}</span></div>
          <div className="report-stat"><span className="report-stat-label">الجلسات</span><span className="report-stat-value">{String(counts.sessions).padStart(2, "0")}</span></div>
          <div className="report-stat"><span className="report-stat-label">المباريات</span><span className="report-stat-value">{String(counts.matches).padStart(2, "0")}</span></div>
          <div className="report-stat"><span className="report-stat-label">سجلات الحضور</span><span className="report-stat-value">{String(counts.attendance).padStart(2, "0")}</span></div>
        </div>
      </section>
